...
class CyclicCosineRestart(keras.callbacks.Callback):
    ...
    def on_train_begin(self, logs={}):
        ...
        if self.use_warmup and self.init_epoch==0:
            self.lr_iter = self.warmup_lr_iter
        elif self.init_epoch>0:
            self.lr_iter = self.continue_lr_iter
        else:
    ...
    def continue_lr(self):
        T_current = np.linspace(self.init_epoch % self.number_of_epochs, self.number_of_epochs,
                                self.number_of_epochs-self.init_epoch % self.number_of_epochs)
        lr_current = self.lr_min + 0.5 * (self.lr_max - self.lr_min) * (
                1 + np.cos((T_current / (self.number_of_epochs)) * np.pi))
        self.continue_lr_iter = itertools.cycle(lr_current.tolist())
    ...
    def on_epoch_begin(self, epoch, logs={}):
        if self.use_warmup and self.init_epoch==0:
            ...
            if (epoch + 1) > ((self.init_epoch//self.number_of_epochs)+1)*self.number_of_epochs:
                self.lr_iter = self.cos_lr_iter
