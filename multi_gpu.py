...
class CheckPointer(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        self.cpu_model.save_weights(file_path, overwrite=True)


class MultiGPUModelCheckpoint(keras.callbacks.Callback):
                ...
                if current is None:
                    ...
                    if self.monitor_op(current, self.best):
                        ...
                        if self.save_weights_only:
                            self.cpu_model.save_weights(filepath,
                                                        overwrite=True)
                            self.weights_path_list[-1]=filepath
                        else:
